...
if ( ! empty($_FILES) || isset($_POST['url']))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		if ($is_img)
		{
			...
			$memory_error = FALSE;
			if ( ! create_img($targetFile, $targetFileThumb, 300, 200))
			{
				$memory_error = TRUE;
			}
			else
...
//				if ($targetPath == '../../uploads/images/'){
//                    darkroom('../../uploads/images/'.$_FILES['file']['name'],'../../thumbs/gallerie/'.$_FILES['file']['name'],0 , 1050 );
//                }
			}
...
function darkroom($img, $to, $width = 0, $height = 0, $useGD = TRUE){
    ...
    if ($useGD) {
        ...
        $background_color = imagecolorallocate ($pattern, 255, 255, 255);
